...
class MatchDetailAwayLineupFragment(val set: Set, val game: Game? = null): Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        leftChevron.setOnClickListener {
            val homeLineupFragment = MatchDetailHomeLineupFragment(game?.home!!, game)
            val supportManager = activity?.supportFragmentManager
            supportManager?.beginTransaction()?.replace(R.id.match_details_body, homeLineupFragment)?.commit()
        }

        rightChevron.visibility = View.INVISIBLE

        rightChevron.setOnClickListener {
            Snackbar.make(goalieImageView, "Away Team Lineup", Snackbar.LENGTH_SHORT).show()
        }
